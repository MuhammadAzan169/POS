...
const QUICK_ASKS = [
  { label: "How is business?", q: "Give me an overall health check of the business right now." },
  {
    ...
    q: "What stock is running low and what exactly should I reorder, with estimated cost?",
  },
  {
    label: "Best sellers",
    q: "What are my best selling and most profitable items in the last 30 days?",
  },
  {
    label: "What isn't selling?",
    q: "Which products are not selling and how much money is tied up in them?",
  },
  {
    label: "Compare my shops",
    q: "Compare my shops on revenue, profit, expenses and net. Which one needs attention?",
  },
  {
    label: "Pricing problems",
    q: "Are any products priced badly — low margin, below cost, or over-discounted?",
  },
  {
    label: "Where is money going?",
    q: "Break down my expenses and tell me if anything looks out of line.",
  },
...
function AIPage() {
  ...
  const ask = async (question: string) => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        { role: "assistant", content: res.answer, model: res.model, failed: !res.ok },
      ]);
      if (!res.ok) {
        ...
        toast.info(
          `Answered by ${res.model} after ${res.attempts.length} model(s) were unavailable`,
        );
      }
    } catch (e) {
      setMessages((prev) => [
        ...prev,
        {
          role: "assistant",
          failed: true,
          content: `Could not reach the assistant: ${e instanceof Error ? e.message : String(e)}`,
        },
      ]);
    } finally {
      setBusy(false);
        ...
        <Card className="p-4 mb-4 flex items-start gap-3 border-warning/40 bg-warning/10">
          ...
          <div className="text-sm">
            ...
            <div className="text-muted-foreground mt-0.5">
              Add <code className="text-xs">OPENROUTER_API_KEY</code> to{" "}
              <code className="text-xs">.env</code> and restart the dev server. The figures below
              still work — they are computed locally.
            </div>
          </div>
        </Card>
      )}
      ...
      <div className="grid gap-3 sm:gap-4 grid-cols-2 lg:grid-cols-4 mb-4 shrink-0">
        <Card className="p-4">
          ...
          <div className="text-2xl font-bold mt-1">
            {money(insights.salesByPeriod.last7Days.revenue)}
          </div>
          <div
            className={`text-xs mt-1 ${trend >= 0 ? "text-success-strong" : "text-destructive"}`}
          >
        ...
        <Card className="p-4">
          ...
          <div className="text-2xl font-bold mt-1">
            {money(insights.salesByPeriod.last7Days.profit)}
          </div>
          <div className="text-xs text-muted-foreground mt-1">
            {insights.salesByPeriod.last7Days.invoices} invoices
          </div>
        </Card>
        <Card className="p-4">
          <div className="text-xs uppercase tracking-wider text-muted-foreground font-medium">
          ...
          <div className={`text-2xl font-bold mt-1 ${lowCount > 0 ? "text-warning-strong" : ""}`}>
            {lowCount}
          </div>
        ...
        <Card className="p-4">
          ...
          <div className="text-xs text-muted-foreground mt-1">
            {insights.catalogue.products} products
          </div>
        </Card>
      </div>
      ...
      <Card className="flex flex-col flex-1 min-h-0">
        <div className="p-4 border-b flex items-center gap-2 shrink-0">
          ...
          {status && status.models.length > 0 && (
            <span
              className="ml-auto hidden sm:flex items-center gap-1.5 text-xs text-muted-foreground"
        ...
        <div
          data-chat-scroll
          className="flex-1 min-h-0 overflow-y-auto overscroll-contain p-3 sm:p-4 space-y-4"
        >
          {messages.length === 0 && (
          ...
          {messages.map((m, i) => (
            <div key={i} className={`flex gap-3 ${m.role === "user" ? "justify-end" : ""}`}>
              ...
              <div
                className={`min-w-0 ${m.role === "user" ? "max-w-[85%] order-first" : "max-w-full sm:max-w-[92%]"}`}
              >
                <div

...
export const Route = createFileRoute("/app/purchases")({
  ...
  ): { restock?: string; shop?: string; qty?: number } => ({
...
function PurchasesPage() {
  ...
  const allRows = useMemo(() => {
    ...
    return visibleShops
      ...
      .sort(
        (a, b) =>
          a.product.name.localeCompare(b.product.name) || a.shop.name.localeCompare(b.shop.name),
      );
  ...
  useEffect(() => {
    ...
    setLines([
      {
        productId: prod.id,
        shopId: shopParam || shops[0]?.id || "",
        qty: Math.max(1, qtyParam ?? 1),
        rate: prod.cost,
      },
    ]);
    setBillNo("");
  ...
  const save = () => {
    ...
    if (lines.some((l) => !l.productId || !l.shopId)) {
      toast.error("Every line needs a product and a shop");
      return;
    }
    ...
    if (
      purchases.some(
        (p) => p.id !== editing?.id && p.billNo.toLowerCase() === billNo.trim().toLowerCase(),
      )
    ) {
    ...
    return (
      <>
        ...
        <Confirm
          ...
          description={
            <>
              {p.lines.reduce((a, l) => a + l.qty, 0)} unit(s) worth{" "}
              {formatRs(p.total, settings.currency)} are taken back off the shelves. If any of them
              have already been sold, correct the bill instead — stock can't go below zero.
            </>
          }
          ...
          onConfirm={() => {
            deletePurchase(p.id);
            toast.success(`Bill ${p.billNo} deleted`);
          }}
          trigger={
            <Button
              size="sm"
      ...
      visiblePurchases.flatMap((p) =>
        p.lines.map((l) => {
          ...
          return [
            ...
            prod?.barcode ?? "",
            shops.find((s) => s.id === l.shopId)?.name ?? "",
            l.qty,
            l.rate,
            l.qty * l.rate,
            p.createdBy ?? "",
      ...
      <Tabs defaultValue="restock">
        ...
        <TabsContent value="restock" className="mt-4">
          <Card className="p-3 sm:p-4 mb-4 grid gap-3 sm:flex sm:flex-wrap sm:items-end">
            ...
            <div className="flex gap-2 sm:ml-auto [&>*]:flex-1 sm:[&>*]:flex-none">
              <Button variant="outline" onClick={exportRestock}>
              ...
              render={(r) => {
                ...
                return (
                  <ListCard
                    ...
                    subtitle={
                      <span className="font-mono">{r.product!.barcode || "No barcode"}</span>
                    }
                    right={r.row.qty}
                    rightSub="in stock"
                    ...
                    actions={
                      <Button
                        size="sm"
                        variant="outline"
                        onClick={() => restock(r.product!.id, r.row.shopId, qty)}
                      >
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {restockRows.map((r) => {
                    ...
                    return (
                      <tr
                        key={`${r.row.productId}-${r.row.shopId}`}
                      ...
                      >
                        ...
                        <td className="px-4 py-3">
                          <StatusPill status={r.row.qty === 0 ? "OUT" : "LOW"} />
                        </td>
        ...
        <TabsContent value="all" className="mt-4">
          <Card className="p-3 sm:p-4 mb-4 grid gap-3 sm:flex sm:flex-wrap sm:items-end">
            ...
            <p className="text-xs text-muted-foreground sm:ml-auto">
              Reorder anything, whether or not it is low.
            </p>
          </Card>
            ...
            <MobileCards
              ...
              render={(r) => (
                <ListCard
                  ...
                  badges={
                    <StatusPill
                      status={r.qty === 0 ? "OUT" : r.qty <= r.product.lowAlert ? "LOW" : "OK"}
                    />
                  }
                  fields={[
                    { label: "Shop", value: r.shop.name },
                  ...
                  actions={
                    <Button
                      ...
                      variant="outline"
                      onClick={() =>
                        restock(r.product.id, r.shop.id, suggestQty(r.qty, r.product.lowAlert))
                      }
                    >
                      <PackagePlus className="h-3.5 w-3.5 mr-1.5" />
                      Restock
        ...
        <TabsContent value="history" className="mt-4">
          <Card className="p-3 sm:p-4 mb-4 flex justify-end">
            <Button variant="outline" className="w-full sm:w-auto" onClick={exportPurchases}>
              <Download className="h-4 w-4 mr-1.5" />
              Export CSV
            </Button>
          </Card>
              ...
              render={(p) => (
                <ListCard
                  ...
                  fields={[
                    ...
                    {
                      label: "Paid",
                      value: formatRs(purchaseSettlement(p).paid, settings.currency),
                    },
                    {
                      label: "Still owed",
                      value:
                  ...
                  {visiblePurchases.map((p) => (
                    <tr key={p.id} className="border-t hover:bg-muted/40">
                      ...
                      <td className="px-4 py-3 text-right font-medium">
                        {formatRs(p.total, settings.currency)}
                      </td>
                      <td className="px-4 py-3 text-right">
                        {purchaseSettlement(p).balance > 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-4">
            <Link to="/app/suppliers" className="block rounded-xl">
              <Card className="p-4 h-full transition-colors hover:bg-muted/40 hover:border-primary/40">
                ...
                <div className="mt-1 text-2xl font-semibold tabular-nums">
                  {formatRs(owedTotal, settings.currency)}
                </div>
            ...
            <Link to={isAdmin ? "/app/ledger" : "/app/suppliers"} className="block rounded-xl">
              <Card className="p-4 h-full transition-colors hover:bg-muted/40 hover:border-primary/40">
                ...
                <div
                  className={`mt-1 text-2xl font-semibold tabular-nums ${overdueTotal > 0 ? "text-destructive" : ""}`}
                >
                ...
                <tbody>
                  {owedBills.map((b) => (
                    <tr key={b.purchase.id} className="border-t hover:bg-muted/40">
                      ...
                      <td className="px-4 py-3 text-right">
                        {formatRs(b.purchase.total, settings.currency)}
                      </td>
                      <td className="px-4 py-3 text-right text-muted-foreground">
                        {formatRs(b.paid, settings.currency)}
                      </td>
                      <td className="px-4 py-3 text-right font-medium text-warning-strong">
                        {formatRs(b.balance, settings.currency)}
                      </td>
                      <td className="px-4 py-3">
                        <BillStatus purchase={b.purchase} />
                      </td>
                      <td className="px-4 py-3 text-right whitespace-nowrap">
                        {billActions(b.purchase, true)}
                      </td>
                    </tr>
                  ))}
                  {owedBills.length === 0 && (
                    <tr>
                      <td
                        ...
                        className="px-4 py-12 text-center text-sm text-muted-foreground"
                      >
                        Nothing owed — every bill is settled.
                      </td>
                    </tr>
                  )}
                </tbody>
          ...
          {owedBills.length > 0 && (
            <Card className="mt-4 p-4">
              ...
              <div className="flex flex-wrap gap-2">
                {[...new Set(owedBills.map((b) => b.purchase.supplierId).filter(Boolean))].map(
                  (sid) => {
      ...
      >
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            {editing && (
              <DialogDescription>
                Stock moves by the difference between what this bill says now and what it said
                before, so nothing is received twice.
              </DialogDescription>
            )}
          </DialogHeader>
          ...
          {addingSupplier && (
            <div className="border rounded-lg p-3 space-y-3 bg-muted/30">
              ...
              <div className="flex gap-2 justify-end">
                ...
                <Button size="sm" onClick={saveNewSupplier}>
                  Add supplier
                </Button>
              </div>
              <p className="text-xs text-muted-foreground">
                Full details (email, address, notes) can be filled in from the Suppliers tab.
              </p>
            </div>
          )}
          ...
          {supplier &&
            !addingSupplier &&
            (supplier.contact || supplier.phone || supplier.notes) && (
                ...
                <p className="text-xs flex items-center gap-1.5 text-success-strong">
                  ...
                  <span className="truncate">
                    ...
                    <span className="text-muted-foreground">
                      {" · "}
                      {scanMatch.barcode || "no barcode"}
                      {" · cost "}
                      {formatRs(scanMatch.cost, settings.currency)}
                    </span>
                  </span>
                </p>
              ) : (
                  ...
                  >
                    <div className="col-span-2 sm:col-span-4">
                      ...
                      <Select
                        ...
                        onValueChange={(v) => {
                          ...
                          setLines((prev) =>
                            prev.map((x, j) =>
                              j === i ? { ...x, productId: v, rate: prod?.cost ?? x.rate } : x,
                            ),
                          );
                        }}
                      >
                    ...
                    <div className="sm:col-span-2">
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          setLines((prev) =>
                            prev.map((x, j) =>
                              j === i ? { ...x, rate: Number(e.target.value) || 0 } : x,
                            ),
                          )
                        }
                      />
                    ...
                    <div className="col-span-2 flex items-center justify-between gap-1 sm:col-span-1 sm:justify-start">
                      ...
                      <button
                        ...
                        title={
                          lines.length === 1 ? "A purchase needs at least one line" : "Remove line"
                        }
                    ...
                    {p && (
                      <div className="col-span-2 -mt-1 text-xs text-muted-foreground sm:col-span-12 sm:pl-1 flex flex-wrap gap-x-3">
                        ...
                        {l.rate > 0 && l.rate !== p.cost && (
                          <span>
                            Cost updates {formatRs(p.cost)} → {formatRs(l.rate)}
                          </span>
                        )}
                      </div>
                    )}
          ...
          */}
          <div className="mt-4 rounded-lg border p-3 space-y-3 bg-muted/20">
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
              ...
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  placeholder="0"
                  onChange={(e) =>
                    setPaidNow(Math.max(0, Math.min(total, Number(e.target.value) || 0)))
                  }
                />
              </div>
              <div className="space-y-1.5">
            ...
            <p className="text-xs text-muted-foreground">
              {billBalance === 0 ? (
                <>
                  Settled in full — nothing is added to what you owe{" "}
                  {supplier?.name ?? "this supplier"}.
                </>
              ) : (
                <>
                  {formatRs(billBalance, settings.currency)} goes onto{" "}
                  {supplier?.name ?? "the supplier"}
                  {"'"}s account
                  {dueDate ? `, due ${dueDate}` : " with no due date set"}. Pay it from the
                  Suppliers tab.
                </>
              )}
